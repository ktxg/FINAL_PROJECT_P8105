...
h1b_employer_commonwealth =  h1b_location %>% 
  filter(employer_state %in% c("DC", "GU",  "MP", "PR", "VI"))

h1b_worksite_commonwealth = h1b_location %>% 
  filter(worksite_state %in% c("DC", "GU", "MH",  "MP", "PR", "VI"))


table(h1b_location$employer_state)

table(h1b_location$worksite_state)
n_distinct(h1b_location$worksite_state)



h1b_employer_commonwealth %>% 
  group_by(employer_state) %>% 
  summarize(n = n()) %>% 
